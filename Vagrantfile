...
VM_NAME = 'master' #PROJECT_NAME + "-" + random_tag()
FORWARD_SSH = true
CUSTOM_PKG_SRC = true
...
unless get_env_var_rb("GUEST_OS")
  set_env_var_rb("GUEST_OS", "ubuntu-1404")
end
...
Vagrant.configure("2") do |config|
  ...
  if PROVISION_WITH_SALT
    ...
    $salt = <<-EOS
      ...
      sudo SSH_AUTH_SOCK=$SSH_AUTH_SOCK salt-call --local grains.setval deescalated_user vagrant --log-level info
      sudo SSH_AUTH_SOCK=$SSH_AUTH_SOCK salt-call --local grains.setval domain test.local --log-level info
      sudo SSH_AUTH_SOCK=$SSH_AUTH_SOCK salt-call --local grains.setval hostname master
